...
public sealed class TrackingCaptureServiceTests
{
    ...
    public void Observe_CompletesPreviousLapAndDownsamplesTrace()
    {
        ...
        Assert.Null(service.Observe(Frame(start, 4, 0.00, 100, 0.1, 0.0, 0.2, 3)));
        Assert.Null(service.Observe(Frame(start.AddMilliseconds(50), 4, 0.05, 120, 0.4, 0.2, -0.1, 4)));
        Assert.Null(service.Observe(Frame(start.AddMilliseconds(100), 4, 0.10, 140, 0.8, 0.5, -0.3, 4)));

        var completed = service.Observe(Frame(start.AddMilliseconds(150), 5, 0.00, 90, 0.0, 0.0, 0.0, 2));

        Assert.NotNull(completed);
        Assert.Equal(4, completed.LapNumber);
