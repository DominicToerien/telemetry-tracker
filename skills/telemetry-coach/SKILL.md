...
3. For a comparison, run `telemetry-tracker laps compare <lap-a> <lap-b> --json`. Inspect a lap summary with `telemetry-tracker laps show <lap-id> --json`; fetch trace detail only when needed with `telemetry-tracker telemetry show --lap <lap-id> --json`.
